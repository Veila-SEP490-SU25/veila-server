...
export class ConvertStoreStringImages1750751703754 implements MigrationInterface {
    ...
    public async up(queryRunner: QueryRunner): Promise<void> {
        ...
        await queryRunner.query(`ALTER TABLE \`users\` CHANGE \`phone\` \`phone\` varchar(15) NULL COMMENT 'Số điện thoại của người dùng'`);
    }

    public async down(queryRunner: QueryRunner): Promise<void> {
        await queryRunner.query(`ALTER TABLE \`users\` CHANGE \`phone\` \`phone\` varchar(15) NULL COMMENT 'Số điện thoại của người dùng (định dạng E.164)'`);
